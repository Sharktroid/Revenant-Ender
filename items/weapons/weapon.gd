...
enum DamageTypes { PHYSICAL, RANGED, MAGIC }
	...
	if not _damage_type:
		match type:
			...
			Types.COBALT_STAFF, Types.CRIMSON_STAFF, Types.LIGHT, Types.DARK, Types.ANIMA:
				_damage_type = DamageTypes.MAGIC
...
func get_weapon_triangle_advantage(weapon: Weapon, _distance: int) -> int:
	return 1 if weapon.type in advantage_types else -1 if weapon.type in disadvantage_types else 0
...
func get_damage_bonus(weapon: Weapon, distance: int) -> int:
	return (
		...
		else get_weapon_triangle_advantage(weapon, distance) if level >= Ranks.S
		else 0
	)
